...
export default function ChatPage() {
  const { doctorId } = useParams({ from: "/patient/chat/$doctorId" });
  ...
  // For demo we can't get a real principal for the sample doctors
  // So we use the caller's own principal as target (self-conversation for demo)
    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] max-w-2xl mx-auto">
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1 max-w-xs">
              Your messages are secure. Start the conversation with{" "}
              {doctor?.name}.
            </p>
          </div>
        ) : (
      ...
      >
        <Input
          ...
          placeholder={`Message ${doctor?.name?.split(" ")[0] ?? "doctor"}...`}
          className="flex-1"
          autoComplete="off"
        />
        <Button
